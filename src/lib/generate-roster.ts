...
export function generateRoster(numPlayers: number): string[] {
    ...
    const goodRoles = Object.values(Roles).filter(role => !roster.includes(role.name) 
    && role.alignment === "GOOD" 
    && role.minPlayers <= numPlayers 
    && role.maxPlayers >= numPlayers)
    const evilRoles = Object.values(Roles).filter(role => !roster.includes(role.name) 
    && role.alignment === "EVIL" 
    && role.minPlayers <= numPlayers 
    && role.maxPlayers >= numPlayers)
    ...
    for (let i = 0; i < numPlayers - initialRoles; i++) {
        if (i % 2 === 0 && goodCount < goodRoles.length) {
            roster.push(goodRoles[goodCount].name)
            goodCount++
        } else if (evilCount < evilRoles.length) {
            roster.push(evilRoles[evilCount].name)
            evilCount++
